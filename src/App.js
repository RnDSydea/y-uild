...
function App() {
  ...
  const [gridComponent, setGridComponent] = useState(
    Array(numColumns * numRows).fill(null)
  );
  ...
  const handleDragStart = (component) => {
    // Genera un ID univoco usando un timestamp
    const uniqueId = `comp-${Date.now()}`;
    // Assegna l'ID al componente e memorizzalo nello stato
    setDraggedComponent({ ...component, id: uniqueId });
  };

  const handleDrop = (index) => {
    if (draggedComponent) {
      const newGridData = [...gridComponent];
      const { lunghezza, altezza, id } = draggedComponent;
      const cellHighlightBorder = calculateCellsToFill(index, lunghezza, altezza);

      // Riempie le celle interessate con il componente trascinato
      cellHighlightBorder.forEach((cellIndex) => {
        if (cellIndex < newGridData.length) {
          newGridData[cellIndex] = draggedComponent;
        }
      });

      setGridComponent(newGridData);
